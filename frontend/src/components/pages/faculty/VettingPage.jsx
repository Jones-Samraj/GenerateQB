...
const VettingPage = () => {
  ...
  useEffect(() => {
    if (!vettingEmail || !user?.course_id) return;
    const fetchFaculty = async () => {
      try {
        ...
        const response = await axios.get(
          ...
          {
            params: { email: vettingEmail, course_id: user.course_id },
            headers: { Authorization: `Bearer ${token}` },
          }
        );
    ...
    fetchFaculty();
  }, [vettingEmail, token, user?.course_id]);

  useEffect(() => {
    if (!vettingEmail || !token || !user?.course_id) return;
    setLoading(true);
    axios
