...
const FacultyDashboard = () => {
  ...
  // Fetch course code based on user.course_id
  useEffect(() => {
    if (user?.course_id) {
      axios
  ...
  useEffect(() => {
    const fetchFacultyData = async () => {
      ...
      if (!email || !user?.course_id) return;

      try {
        const response = await axios.get(
          `http://localhost:7000/api/faculty/faculty-data?email=${email}&course_id=${user.course_id}`,
          { headers: { Authorization: `Bearer ${token}` } }
        );
    ...
    fetchFacultyData();
  }, [dispatch, token, user?.email, user?.course_id]);

  // Fetch weekly/monthly stats once course_id is available
  useEffect(() => {
    if (user?.course_id) {
      axios
